            ...
            @foreach ($reservas as $reserva)
                <div class="reserva-card {{ $reserva->estado }}">
                    ...
                    <div class="puesto-info">
                        <p><i class="fa-solid fa-square-parking"></i> {{ __('messages.spot') }}: <strong>{{ $reserva->puesto->numero_puesto ?? 'N/A' }}</strong></p>
                        ...
                        <p class="estado estado-{{ $reserva->estado }}">{{ ucfirst($reserva->estado) === 'Pendiente' ? __('messages.pending') : (ucfirst($reserva->estado) === 'Aceptado' ? __('messages.accepted') : __('messages.rejected')) }}</p>
                    </div>

                    <!-- 📅 Fechas -->
                    <div class="fecha-info">
                        <p><i class="fa-regular fa-clock"></i> <strong>{{ __('messages.requested') }}:</strong> {{ $reserva->created_at->format('d/m/Y H:i') }}</p>
                        @if($reserva->estado === 'aceptado' && $reserva->hora_aprobacion)
                            <p><i class="fa-regular fa-calendar-check"></i> <strong>{{ __('messages.approved') }}:</strong> {{ \Carbon\Carbon::parse($reserva->hora_aprobacion)->format('d/m/Y H:i') }}</p>
                        @endif
                    </div>
                    ...
                    @elseif ($reserva->estado === 'rechazado')
                        <p class="mensaje-rechazo"><i class="fa-solid fa-circle-exclamation"></i> {{ __('messages.reservation_rejected') }}</p>
                    @endif
                </div>
            @endforeach
        </div>
